(async function () {
  ...
  document.getElementById('gallery-grid').innerHTML = data.map(item => {
    ...
    return `<div class="tile"><a href="${assetBase}${esc(item.local_path)}"><img src="${assetBase}${esc(item.local_path)}" loading="lazy" alt=""></a>${usageHtml}<div class="small">${esc(item.original_url || '')}</div></div>`;
  }).join('');
})();
